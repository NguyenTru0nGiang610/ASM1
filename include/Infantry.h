...
public:
    ...
    static InfantryType stringToInfantryType(string it) {
        if (it == "SNIPER") return SNIPER;
        if (it == "ANTIAIRCRAFTSQUAD") return ANTIAIRCRAFTSQUAD;
        if (it == "MORTARSQUAD") return MORTARSQUAD;
        if (it == "ENGINEER") return ENGINEER;
        if (it == "SPECIALFORCES") return SPECIALFORCES;
        return REGULARINFANTRY;
    }
    Infantry (const Infantry &other) : Unit(other), infantryType(other.infantryType) {}
};
