...
bool UNIT_TEST_Task3::Task303() {
    ...
    Infantry* sniper = new Infantry(10, 5, Position(0, 1), SNIPER); // Within radius 2  60
    Infantry* antiarcraftsquad = new Infantry(8, 6, Position(0, 2), ANTIAIRCRAFTSQUAD); // Within radius 2 116
    Infantry* mortarsquad = new Infantry(6, 7, Position(0, 3), MORTARSQUAD); // 154
    Vehicle* truck = new Vehicle(10, 5, Position(0, 0), TRUCK); // Within radius 2  2 
    Vehicle* mortar = new Vehicle(8, 6, Position(0, 1), MORTAR); // Within radius 2  12
    Vehicle* antiaircraft = new Vehicle(6, 7, Position(0, 2), ANTIAIRCRAFT); // Within radius 2  22
    Vehicle* armoredcar = new Vehicle(5, 8, Position(0, 3), ARMOREDCAR); //   32
    Infantry* engineer = new Infantry(5, 8, Position(0, 4), ENGINEER); //  208   
    Infantry* specialforces = new Infantry(4, 9, Position(0, 5), SPECIALFORCES); //  260
    Infantry* regularinfantry = new Infantry(3, 10, Position(0, 0), REGULARINFANTRY); // Within radius 320
    Vehicle* apc = new Vehicle(4, 9, Position(0, 4), APC); //    42
    Vehicle* artillery = new Vehicle(3, 10, Position(0, 5), ARTILLERY);// 52
    Vehicle* tank = new Vehicle(2, 11, Position(0, 6), TANK); // 62
